...
def homepage():
    ...
    return (
        f"<title>Home</title>Welcome to the Weather API for Honolulu, Hawaii!"
...
def precipitation():
    ...
    # Creates session and queries from the Measurement class the date and precipitation for each station, ordered by date.
    ...
    # Parses through the results and creates a dictionary to be output.
    all_results = []
    for item in results:
        item_dict = {}
    ...
    # Returns the dictionary, jsonified.
...
def stations():
   ...
   # Creates session and queries from the Station class, station name and id, ordered by name.
    session = Session(engine)
    results = session.query(Station.station, Station.name).order_by(Station.name).all()
    session.close()

    # Creates two outputs. One is a list of just the IDs. The other is a dictionary of the name and ID together.
    all_results = []
    station_list = []
    for item in results:
        item_dict = {}
    ...
    # Returns first the list and then the dictionary, both jsonified.
...
def temperatures():
    ...
    # Creates a session and queries the last year of dates and temperatures only from the most active station (ma).
    session = Session(engine)
    one_year = dt.datetime(2016, 8, 23)
    ...
    # Iterates through results to create dictionary of date and temperatures for station (ma).
    all_results = []
    for item in results:
        item_dict = {}
        item_dict["Date"] = item[0]
...
def since_date(start_date):
    ...
    return (
        f"<title>{start_date}</title><u>Temperature Data Since {start_date}</u>"
...
def between_dates(start_date,end_date):
    ...
    # Pulls year, month, and day values for the input start and end date.
    start_year = int(start_date[0:4])
    start_month = int(start_date[5:7])
    start_day = int(start_date[8:10])
    ...
    # Makes sure the input start date is the right length.
    if len(start_date) != 10:
    ...
    # Makes sure the input end date is the right length.
    if len(end_date) != 10:
    ...
    # Tries standard query but accounts for ValueError.
    try:

        # Creates session and queries for temperature between given dates.
        session = Session(engine)
        results = session.query(Measurement.tobs).\
        ...
        # Creates new list without 'Null' temperatures.
        temp_list = []
        for x in results:
            if x[0] != 'Null':
                temp_list.append(float(x[0]))

        # Calculates max, min, and average temperature from the list.
        max_temp = max(temp_list)
        min_temp = min(temp_list)
        avg_temp = round(st.mean(temp_list),1)

    # Returns error message if the start date is after the end date in any fashion or other error message if they somehow
    #   entered a date that made it through all the error handling.
    except ValueError:
    ...
    # Returns temperature data - finally!
    return (
        f"<title>{start_date} to {end_date}</title><u>Temperature Data Between {start_date} and {end_date}</u>"
        f"</br>Max Temperature:    {max_temp}°F"
        f"</br>Min Temperature:    {min_temp}°F"
        f"</br>Average Temperature:    {avg_temp}°F"
    )

# Runs app and debugger.
